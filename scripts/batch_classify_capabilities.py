...
Run AI capability classification on all entities that don't have capabilities yet.
"""
...
def main():
    ...
    logger.info("🤖 Starting Batch Capability Classification...")
    ...
    for i, entity in enumerate(entities_without_capabilities):
        ...
        try:
            # Get entity's raw data for classification
            raw_data_query = "SELECT content FROM raw_data WHERE entity_id = ?"
            raw_data_results = db.db_manager.execute_query(raw_data_query, (entity.id,))
            
            if not raw_data_results:
                logger.warning(f"No raw data found for {entity.name}. Skipping.")
                failed_count += 1
                continue
            
            # Combine all raw data content
            combined_content = " ".join([row['content'] for row in raw_data_results])
            
            # Classify capabilities
            capabilities = classifier.classify_capabilities(combined_content)
    ...
    # Summary
    logger.info("🎉 Batch Capability Classification Complete!")
