"""Semiconductor Digest collector for CHIPS Act intelligence."""
...
class SemiconductorDigestCollector(BaseCollector):
    """Collector for Semiconductor Digest articles."""
    
    def __init__(self):
        super().__init__()
        self.base_url = "https://www.semiconductor-digest.com"
        self.search_url = "https://www.semiconductor-digest.com/search"
        
    def get_source_name(self) -> str:
        """Get the source name."""
        return "semiconductor_digest"
    
    def collect(self) -> List[Dict[str, Any]]:
        """Collect CHIPS Act related articles from Semiconductor Digest."""
        logger.info("Starting Semiconductor Digest collection...")
        ...
        logger.info(f"Semiconductor Digest collection completed. Found {len(collected_data)} articles.")
    ...
    def _search_articles(self, keyword: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not response or response.status_code != 200:
                logger.warning(f"Failed to search Semiconductor Digest for {keyword}")
                return articles
            ...
            logger.error(f"Error searching Semiconductor Digest: {e}")
    ...
    def _parse_search_results(self, html_content: str, keyword: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error parsing Semiconductor Digest results: {e}")
