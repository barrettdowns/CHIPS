"""
University press release collector for CHIPS Act related announcements.
"""
...
class UniversityPressCollector(BaseCollector):
    """Collector for university press releases related to CHIPS Act."""
    ...
    def __init__(self):
        ...
        self.university_sources = [
            ...
            {
                'name': 'University of Michigan News',
            ...
            {
                'name': 'Arizona State University News',
                'url': 'https://news.asu.edu',
                'search_path': '/search',
                'article_selector': '.article, .news-item'
            },
            {
                'name': 'University of Texas News',
    ...
    def get_source_name(self) -> str:
        ...
        return "University Press Releases"
    
    def collect(self) -> List[Dict[str, Any]]:
        """Collect CHIPS Act related press releases from universities."""
        logger.info("Starting university press release collection...")
        
        collected_data = []
        
        for source in self.university_sources:
            try:
                logger.info(f"Collecting from {source['name']}...")
                
                # Search for CHIPS Act related content
                articles = self._search_university_news(source)
        ...
        logger.info(f"University press release collection completed. Found {len(collected_data)} entities.")
        return collected_data
    
    def _search_university_news(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Search for CHIPS Act related news from a university."""
        articles = []
    ...
    def _extract_entity_from_article(self, article: Dict[str, Any], source: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # Look for partner companies in the content
        partner_companies = self._extract_partner_companies(article['content'])
        ...
        university_entity = {
            ...
            'confidence_score': 0.9,  # High confidence for university press releases
        ...
        entities.append(university_entity)
        
        # Add partner company entities
        for company in partner_companies:
            company_entity = {
                'name': company,
                'legal_name': company,
                'entity_type': EntityType.COMPANY.value,
                'confidence_score': 0.7,  # Medium confidence for mentioned companies
                'source_url': article['url'],
                'source_data': {
                    'article_title': article['title'],
                    'article_date': article['date'],
                    'mentioned_in': university_name,
                    'university_url': source['url']
                },
                'funding': self._extract_funding_from_content(article['content']),
                'capabilities': self._extract_capabilities_from_content(article['content'])
    ...
    def _extract_partner_companies(self, content: str) -> List[str]:
        """Extract partner company names from content."""
        companies = []
            ...
            for match in matches:
                ...
                try:
                    ...
                    funding_info.append({
                        ...
                        'description': f"Funding mentioned in press release: {match.group(0)}"
                    })
